        ...
        <div class="page-wrapper">
            <div class="dashboard-page">
                <aside class="dashboard-sidebar">
                    ...
                    <label for="dashboard-menu-toggle" class="dashboard-burger" aria-label="Ouvrir le menu"><span></span><span></span><span></span></label>
                    ...
                    <div class="dashboard-sidebar-bottom">
                        <a href="{{ route('profil') }}" class="nav-link is-active">
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <header class="dashboard-header">
                        <div>
                            <h1 class="heading-style-h3-bold">
                                Profil</h1>
                            <p class="text-color-body text-size-small">Gérer les informations de votre espace personnel.</p>
                        </div>
                        ...
                        <section class="client-section">
                            <article class="dashboard-card entity-form-card">
                                @if (session('status') === 'profile-updated')
                                    <div class="w-form-done" style="display: block;">Profil mis à jour.</div>
                                @endif
                                ...
                                <form class="dashboard-form" method="POST" action="{{ route('profil.update') }}">
                                    @csrf
                                    @method('PATCH')
                                    <label>Nom<input name="nom" type="text" value="{{ old('nom', $user->nom) }}" required></label>
                                    <label>Prénom<input name="prenom" type="text" value="{{ old('prenom', $user->prenom) }}" required></label>
                                    <label>Email<input name="email" type="email" value="{{ old('email', $user->email) }}" required></label>
                                    <label>Téléphone<input name="tel" type="tel" value="{{ old('tel', $user->tel) }}" required></label>
                                    <button type="submit" class="dashboard-action-button">Mettre à jour</button>
                                </form>
                            </article>
                        </section>
                    </section>
