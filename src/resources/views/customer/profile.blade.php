...
<form method="POST" action="{{ route('customer.profile.info') }}">
    @csrf
    @method('PUT')
...
<p>Attention : cette action supprime aussi vos comptes, revenus et depenses.</p>

<form method="POST"
      action="{{ route('customer.profile.delete') }}"
