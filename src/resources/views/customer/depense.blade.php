            ...
            <div class="dashboard-page">
                <aside class="dashboard-sidebar">
                    ...
                    <nav class="dashboard-nav">
                        ...
                        <a href="{{ route('customer.depenses.index') }}" class="nav-link is-active">
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <section class="dashboard-content client-sections-content">
                        ...
                        <section class="client-section">
                            <article class="dashboard-card entity-form-card">
                                <h3 class="heading-style-h5-bold">Creer une depense</h3>
                            ...
                            <div class="entity-grid">
                                @foreach($taxDepenses as $taxDepense)
                                    <article class="dashboard-card entity-card">
                                        <span class="entity-badge is-once">Automatique</span>
                                        <h3 class="heading-style-h5-bold">Taxe sur interets</h3>
                                        <p class="text-color-body">Calculee avec le taux d'imposition du compte.</p>
                                        <dl class="entity-details">
                                            <div><dt>Compte</dt><dd>{{ $taxDepense['account']->name }}</dd></div>
                                            <div><dt>Taxe annuelle</dt><dd>{{ number_format($taxDepense['montant'], 2, ',', ' ') }} €</dd></div>
                                            <div><dt>Interets bruts</dt><dd>{{ number_format($taxDepense['interets'], 2, ',', ' ') }} €</dd></div>
                                            <div><dt>Taux</dt><dd>{{ number_format($taxDepense['account']->tax_rate, 2, ',', ' ') }} %</dd></div>
                                        </dl>
                                    </article>
                                @endforeach
                                ...
                                @foreach($depenses as $depense)
                                    <article class="dashboard-card entity-card">
                                        <span class="entity-badge {{ $depense->fractionnement === 'unique' ? 'is-once' : '' }}">{{ ucfirst($depense->fractionnement) }}</span>
                                        <h3 class="heading-style-h5-bold">{{ $depense->nom }}</h3>
                                        <p class="text-color-body">{{ $depense->description ?? 'Aucune description' }}</p>

                                        <dl class="entity-details">
                                            <div><dt>Compte</dt><dd>{{ $depense->account?->name ?? '-' }}</dd></div>
                                            <div><dt>Montant</dt><dd>{{ number_format($depense->montant, 2, ',', ' ') }} €</dd></div>
                                            <div><dt>Date d'effet</dt><dd>{{ \Carbon\Carbon::parse($depense->date_effet)->format('d/m/Y') }}</dd></div>
                                            <div><dt>Dernier debit</dt><dd>{{ $depense->last_debited_at ? \Carbon\Carbon::parse($depense->last_debited_at)->format('d/m/Y') : '-' }}</dd></div>
                                        </dl>
                                        ...
                                        <details class="entity-edit">
                                            ...
                                            <form class="dashboard-form" method="POST" action="{{ route('customer.depenses.update', $depense) }}">
                                                @csrf
                                                @method('PUT')
                                                <label>Nom<input name="nom" type="text" value="{{ old('nom', $depense->nom) }}" required></label>
                                                <label>Description<input name="description" type="text" value="{{ old('description', $depense->description) }}"></label>
                                                <label>Compte
                                                    <select name="account_id" required>
                                                        @foreach($accounts as $account)
                                                            <option value="{{ $account->id }}" @selected(old('account_id', $depense->account_id) == $account->id)>{{ $account->name }}</option>
                                                        @endforeach
                                                    </select>
                                                </label>
                                                <label>Montant<input name="montant" type="number" step="0.01" min="0" value="{{ old('montant', $depense->montant) }}" required></label>
                                                <label>Date d'effet<input name="date_effet" type="date" value="{{ old('date_effet', \Carbon\Carbon::parse($depense->date_effet)->format('Y-m-d')) }}" required></label>
                                                <label>Fractionnement
                                                    <select name="fractionnement" required>
                                                        <option value="mensuel" @selected(old('fractionnement', $depense->fractionnement) === 'mensuel')>Mensuel</option>
                                                        <option value="semestriel" @selected(old('fractionnement', $depense->fractionnement) === 'semestriel')>Semestriel</option>
                                                        <option value="annuel" @selected(old('fractionnement', $depense->fractionnement) === 'annuel')>Annuel</option>
                                                        <option value="unique" @selected(old('fractionnement', $depense->fractionnement) === 'unique')>Unique</option>
                                                    </select>
                                                </label>
                                        ...
                                        <div class="entity-actions">
                                            <form method="POST" action="{{ route('customer.depenses.delete', $depense) }}" data-confirm="Supprimer cette depense ?">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @if($depenses->isEmpty() && $taxDepenses->isEmpty())
