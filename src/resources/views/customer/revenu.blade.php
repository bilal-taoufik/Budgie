                    ...
                    <section class="dashboard-content client-sections-content">
                        ...
                        <section class="client-section">
                            <article class="dashboard-card entity-form-card">
                                <h3 class="heading-style-h5-bold">Creer un revenu</h3>

                                <form class="dashboard-form" method="POST" action="{{ route('customer.revenues.store') }}">
                                    @csrf
                                    <label>Nom<input name="revenu_nom" type="text" value="{{ old('revenu_nom') }}" required></label>
                                    <label>Description<input name="revenu_description" type="text" value="{{ old('revenu_description') }}"></label>
                                    <label>Compte
                                    ...
                                    <label>Montant<input name="revenu_montant" type="number" step="0.01" min="0" value="{{ old('revenu_montant') }}" required></label>
                                    ...
                                    <label>Date d'effet<input name="revenu_date_effet" type="date" value="{{ old('revenu_date_effet', now()->format('Y-m-d')) }}" required></label>
                                    <button type="submit" class="dashboard-action-button">Enregistrer</button>
                                </form>
                            </article>

                            <div class="entity-grid">
                                @foreach($interestRevenues as $interestRevenue)
                                    <article class="dashboard-card entity-card">
                                        <span class="entity-badge is-once">Automatique</span>
                                        <h3 class="heading-style-h5-bold">Interets</h3>
                                        <p class="text-color-body">Calcules avec le taux de remuneration du compte.</p>
                                        <dl class="entity-details">
                                            <div><dt>Compte</dt><dd>{{ $interestRevenue['account']->name }}</dd></div>
                                            <div><dt>Brut annuel</dt><dd>{{ number_format($interestRevenue['montant'], 2, ',', ' ') }} €</dd></div>
                                            <div><dt>Taxes estimees</dt><dd>{{ number_format($interestRevenue['taxes'], 2, ',', ' ') }} €</dd></div>
                                            <div><dt>Net estime</dt><dd>{{ number_format($interestRevenue['net'], 2, ',', ' ') }} €</dd></div>
                                        </dl>
                                    </article>
                                @endforeach
                                ...
                                @foreach($revenues as $revenue)
                                    <article class="dashboard-card entity-card">
                                        <span class="entity-badge {{ $revenue->revenu_fractionnement === 'unique' ? 'is-once' : '' }}">{{ ucfirst($revenue->revenu_fractionnement) }}</span>
                                        <h3 class="heading-style-h5-bold">{{ $revenue->revenu_nom }}</h3>
                                        <p class="text-color-body">{{ $revenue->revenu_description ?? 'Aucune description' }}</p>

                                        <dl class="entity-details">
                                            <div><dt>Compte</dt><dd>{{ $revenue->account?->name ?? '-' }}</dd></div>
                                            <div><dt>Montant</dt><dd>{{ number_format($revenue->revenu_montant, 2, ',', ' ') }} €</dd></div>
                                            <div><dt>Date d'effet</dt><dd>{{ \Carbon\Carbon::parse($revenue->revenu_date_effet)->format('d/m/Y') }}</dd></div>
                                            <div><dt>Dernier credit</dt><dd>{{ $revenue->last_credited_at ? \Carbon\Carbon::parse($revenue->last_credited_at)->format('d/m/Y') : '-' }}</dd></div>
                                        </dl>
                                        ...
                                        <details class="entity-edit">
                                            ...
                                            <form class="dashboard-form" method="POST" action="{{ route('customer.revenues.update', $revenue) }}">
                                                @csrf
                                                @method('PUT')
                                                <label>Nom<input name="revenu_nom" type="text" value="{{ old('revenu_nom', $revenue->revenu_nom) }}" required></label>
                                                <label>Description<input name="revenu_description" type="text" value="{{ old('revenu_description', $revenue->revenu_description) }}"></label>
                                                <label>Compte
                                                    <select name="account_id" required>
                                                        @foreach($accounts as $account)
                                                ...
                                                <label>Montant<input name="revenu_montant" type="number" step="0.01" min="0" value="{{ old('revenu_montant', $revenue->revenu_montant) }}" required></label>
                                                <label>Date d'effet<input name="revenu_date_effet" type="date" value="{{ old('revenu_date_effet', \Carbon\Carbon::parse($revenue->revenu_date_effet)->format('Y-m-d')) }}" required></label>
                                                <label>Fractionnement
                                                    <select name="revenu_fractionnement" required>
                                                        <option value="mensuel" @selected(old('revenu_fractionnement', $revenue->revenu_fractionnement) === 'mensuel')>Mensuel</option>
                                                        <option value="semestriel" @selected(old('revenu_fractionnement', $revenue->revenu_fractionnement) === 'semestriel')>Semestriel</option>
                                                        <option value="annuel" @selected(old('revenu_fractionnement', $revenue->revenu_fractionnement) === 'annuel')>Annuel</option>
                                                        <option value="unique" @selected(old('revenu_fractionnement', $revenue->revenu_fractionnement) === 'unique')>Unique</option>
                                                    </select>
                                                </label>
                                        ...
                                        <div class="entity-actions">
                                            <form method="POST" action="{{ route('customer.revenues.delete', $revenue) }}" data-confirm="Supprimer ce revenu ?">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @if($revenues->isEmpty() && $interestRevenues->isEmpty())
                                    <article class="dashboard-card entity-card"><p class="text-color-body">Aucun revenu pour le moment.</p></article>
                                @endif
                            </div>
                        </section>
                    </section>
